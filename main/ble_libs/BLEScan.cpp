...
bool BLEScan::start(uint32_t duration, void (*scanCompleteCB)(BLEScanResults), bool is_continue) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gap_set_scan_params: err: %d, text: ", errRc);
		m_semaphoreScanEnd.give();
		return false;
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gap_start_scanning: err: %d, text: ", errRc);
		m_semaphoreScanEnd.give();
		return false;
	}
...
void BLEScan::stop() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gap_stop_scanning: err: %d, text: ", errRc);
		return;
	}
