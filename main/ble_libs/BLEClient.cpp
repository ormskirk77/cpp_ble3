...
// and then block on its completion.  When the event has arrived, we will have the handle.
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_app_register: rc=%d", errRc);
		return false;
	}
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_open: rc=%d", errRc);
		return false;
	}
...
void BLEClient::disconnect() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_close: rc=%d", errRc);
		return;
	}
	ESP_LOGD(LOG_TAG, "<< disconnect()");
} // disconnect
...
void BLEClient::gattClientEventHandler(
	...
	switch(event) {
		...
		case ESP_GATTC_CONNECT_EVT: {
			...
			if (errRc != ESP_OK) {
				ESP_LOGE(LOG_TAG, "esp_ble_gattc_send_mtu_req: rc=%d", errRc);
			}
...
// TODO implement retrieving services from cache
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_search_service: rc=%d ", errRc);
