...
std::string BLERemoteCharacteristic::readValue() {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_read_char: rc=%d ", errRc);
		return "";
	}
...
void BLERemoteCharacteristic::registerForNotify(notify_callback notifyCallback, bool notifications) {
	...
	if (notifyCallback != nullptr) {   // If we have a callback function, then this is a registration.
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_ble_gattc_register_for_notify: rc=%d ", errRc);
		}
		...
		if (errRc != ESP_OK) {
			ESP_LOGE(LOG_TAG, "esp_ble_gattc_unregister_for_notify: rc=%d ", errRc);
		}
...
void BLERemoteCharacteristic::writeValue(uint8_t* data, size_t length, bool response) {
	...
	if (errRc != ESP_OK) {
		ESP_LOGE(LOG_TAG, "esp_ble_gattc_write_char: rc=%d ", errRc);
		return;
	}
